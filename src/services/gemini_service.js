...
class GeminiService {
    ...
    _finalizeReplyQuality(text = '') {
        ...
        // Optimización: Si la respuesta es demasiado larga para WhatsApp (>3000 chars), recortar y cerrar.
        if (out.length > 3200) {
            out = out.slice(0, 3100) + '... (Mensaje simplificado por longitud)';
        }
    ...
    async generateResponse(userMessage, chatHistory, userName = '', userGeminiConfig = null, userPhone = '') {
        ...
        while (attempts < maxAttempts) {
            ...
            const behaviorPrompt = [
                ...
                "Evita usar reglas fijas de formato ni saltos de linea excesivos.",
                "Haz que el mensaje sea MUY conciso. La legibilidad en WhatsApp requiere textos cortos.",
                "Tienes acceso a un 'SUPER CATÁLOGO' general y, a veces, a 'INFO DETALLADA'.",
                "Si el usuario SOLO saluda (ej: 'hola', 'buenos días', 'buenas tardes'), responde ÚNICAMENTE con un saludo cordial y pregúntale en qué le puedes ayudar o sobre qué producto desea información. NO envíes listas de productos ni el catálogo en un simple saludo.",
                ...
                "Si el usuario pregunta por un producto específico, dale una breve descripción y enumera de forma clara los planes disponibles JUNTO CON UN MUY BREVE RESUMEN DE CADA PLAN (1 línea máximo), sus CICLOS DE FACTURACIÓN y precios respectivos.",
                "REGLA OBLIGATORIA DE ESTRUCTURA: Los usuarios a menudo se refieren a los 'ciclos de facturación' (1 mes, 3 meses, anual, etc) como 'planes de cada plan'. Si el producto tiene ciclos o periodos de tiempo con precios diferentes (Ej: 1 mes, 3 meses, 6 meses, anual), LISTA todos esos ciclos con sus precios de forma limpia para que el usuario pueda compararlos directamente. No ocultes los precios ni los ciclos.",
                "REGLA DE CONTENIDO: No envíes muros de texto describiendo las características o todo lo que 'incluye' el plan, a menos que el usuario lo pida explícitamente. Enfócate en mostrar las opciones de planes, ciclos (meses) y precios.",
                "Si el usuario no quiere continuar, cierra con respeto y sin insistencia comercial.",
                "REGLA DE PRECIOS OBLIGATORIA: cuando exista 'precio' y 'precio_original', el monto real a pagar SIEMPRE es 'precio'."
            ].join(' ');
